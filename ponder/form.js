...
function updateNotesField() {
  ...
  if (value === "many") {
    ...
    notes.value = ""; // clear the field when hidden
  }
}
...
form.addEventListener("submit", function (event) {
  ...
  if (type === "many" && selectedCampuses.length < 2) {
    output.textContent = "You selected 'Two or more campuses' — please check at least two campuses.";
    return;
  }

  // Let the user know if they choose many campuses but didn't put a note that they need to add a note
  if (type === "many" && note === "") {
    output.textContent = "Please add travel notes when selecting two or more campuses.";
    return;
  }
